...
let allMessages = []; // Store all messages for the current chat session
...
function joinChat(matchId, matchName) {
    ...
    document.getElementById("chat-with").innerHTML = "Chat with " + '<a id="chat-profile-link">' + matchName + "</a>";   // Also link to users profile

    // Add listener for clicking the users name that then gives you the users profile
    document.getElementById("chat-profile-link").addEventListener("click", function(event) {
    ...
    document.getElementById('filter-button').addEventListener('click', filterMessages);

    // Add listener for the clear filters button
...
// Added Part: Function to filter messages based on a keyword
function filterMessages() {
    console.log(filter);
    const keyword = document.getElementById('message-filter').value.toLowerCase();
    if(keyword.trim() == "") {
        displayMessages(allMessages);
    } else {
        const filteredMessages = allMessages.filter(msg => msg.message.toLowerCase().includes(keyword));
